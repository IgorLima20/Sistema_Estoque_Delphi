...
begin
  ...
  2:begin
    caminho := ExtractFilepath(Application.ExeName);
    if Frm_pesq_geral_mes.Rel_Geral.LoadFromFile(caminho + 'Rel_Falta_pagar_Falta_Receber_mes.fr3') then
    begin
      Rel_Geral.clear;
      Rel_Geral.LoadFromFile(extractfilepath(application.ExeName) +  'Rel_Falta_pagar_Falta_Receber_mes.fr3');
      Rel_Geral.Variables['data_inicial'] := QuotedStr(MK_Inicio.Text);
      Rel_Geral.Variables['data_final'] := QuotedStr(MK_Fim.Text);
      Rel_Geral.Variables['usuario'] := QuotedStr(DM.Usuario);
      Rel_Geral.prepareReport(true);
