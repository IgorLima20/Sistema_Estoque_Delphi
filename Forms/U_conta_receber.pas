...
uses
  ...
  Vcl.Buttons, Vcl.ExtCtrls, Vcl.Mask, frxClass, frxDBSet;
...
begin
  ...
  if Frm_conta_receber.Rel_recibo.LoadFromFile(caminho + 'Recibo_Pagamento.fr3') then
  begin
    Rel_recibo.clear;
    Rel_recibo.LoadFromFile(extractfilepath(application.ExeName) +  'Recibo_Pagamento.fr3');
    Rel_recibo.prepareReport(true);
    Rel_recibo.ShowPreparedReport;
  end
  else
